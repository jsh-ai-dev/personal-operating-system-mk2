...
import { getMk3ServiceUrl } from "@/lib/server/mk3ServiceUrl";
...
async function proxy(request: NextRequest, pathParts: string[]): Promise<NextResponse> {
  ...
  const target = `${getMk3ServiceUrl()}/api/${subPath}${request.nextUrl.search}`;
  ...
  try {
    ...
    return NextResponse.json(
      { message: "mk3 API 서버에 연결할 수 없습니다. MK3_SERVICE_URL과 mk3 backend(기본 8001) 실행 여부를 확인하세요." },
      { status: 502 },
    );
  }
